...
//for creating a collection (CREATE)
...
//for getting all datas (READ)
const querySnapshot = await getDocs(collection(db, "cities"));
querySnapshot.forEach((doc) => {
  // console.log(doc.id, " => ", doc.data());
});

// Remove the 'capital' field from the document (UPDATE)
...
function App() {
  ...
  return (
    <div className="App">
      <button
        onClick={() =>
          createUserWithEmailAndPassword(auth, "sreera@gmail.com", "sreerag")
            .then((userCredential) => {
              // Signed up (create user)
